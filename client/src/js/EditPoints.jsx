...
class EditPoints extends Component {
  ...
  submitPoints = () => {
    ...
    const requestOptions = {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({id: this.state.username, points: this.state.points, message: this.state.message})
    };
    fetch(this.props.url + "/setCurrentScore", requestOptions);
    this.setState({pointsEdited: false, message: ""});
  }
            ...
            <Row>
              <Col xs={12} md={6} className="">
                ...
                <div className="input-group justify-content-center align-items-center" >
                  <Button className=" m-1" variant="primary" onClick={() =>{
                    this.setState({pointsEdited:true, points: this.state.points + this.state.currentGameWonSelected})}}> Add Points</Button> 
                    
                  <Button className=" m-1" variant="info" onClick={() =>{

                    this.setState({pointsEdited:true, points: this.state.points - this.state.currentGameWonSelected})}
              ...
              <Col xs={12} md={6} > 
                ...
                <div className="display-4 text-primary text-center">
                  {this.state.pointsEdited ? "Your new points will be" : "Your current points are:"} <span className="text-success"> {this.state.points}</span>
                </div>
                <input type="text" placeholder="Enter Message" onChange={ (e) => this.setState({message: e.target.value})} value={this.state.message} className="form-group w-100  form-control"/>
